...
from utility import delete_file, append_file, get_target_pos_names, list_all_files, load_matrix, save_matrix
...
def batch_one_image_dataset(global_output_path, img_window_path, img_vlad_path, img_metadata_path, target, overlap_threshold=0.5):
    ...
    global_X_path = global_output_path + "global_X.txt"
    global_Y_path = global_output_path + "global_Y.txt"
...
def batch_all_images(input_image_path, annotation_path, output_parent_path,
    ...
    for i in xrange(len(image_name_list)):
        ...
        vlad_vector_batch(window_sift_path=sift_path_L[i], output_path=output_path + image_name, vocabulary=vocabulary)
    ...
    if dataset_mode:
        ...
        for img_name in image_name_list:
            ...
            metadata_path = '%s%s.xml' % (annotation_path, image_name)
            batch_one_image_dataset(output_parent_path, img_window_path,
                        img_vlad_path, metadata_path, target, overlap_threshold=overlap_threshold)
        print "\tData set prepared done"
    print "....................All done"
